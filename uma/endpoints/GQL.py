...
class TaskPlanInput:
    ...
    conversation_name: str = "uma Task Planning"
...
class Mutation:
    ...
    async def learn_file(self, info, agent_name: str, input: FileMemoryInput) -> bool:
        ...
        if input.company_id:
            uma = auth_manager.get_company_agent_session(company_id=input.company_id)
            response = uma.learn_file(
                agent_name="uma",
    ...
    async def submit_feedback(
        ...
        conversation = uma.conversation
        if conversation.has_received_feedback(message=input.message):
            return False

        memory = (
            uma.agent_interactions.agent_memory
            if input.positive
            else uma.agent_interactions.agent_memory
        )

        reflection = await uma.inference(
            user_input=input.user_input,
...
"""
    ...
    async def generate_dpo(self, info, agent_name: str, input: DPOInput) -> DPOResult:
        ...
        prompt, chosen, rejected = await uma.dpo(
