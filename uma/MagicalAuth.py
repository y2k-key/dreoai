...
- uma_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- uma_URI: URL to the uma server
...
def is_uma_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("uma_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if uma_API_KEY:
        ...
        logging.error(
            "uma API Key is missing. Please set the uma_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("uma_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                uma = umaSDK(base_uri=getenv("uma_URI"))
                uma.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                uma.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                uma.add_agent(
                    agent_name="uma",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        uma = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> umaSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> umaSDK:
        if not company_id:
            company_id = self.company_id
        ...
        uma.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return uma
