...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await xdai.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        xdai = auth.get_company_agent_session(company_id=external_source.company_id)
        response = xdai.delete_memory_external_source(
            agent_name="xdai",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    xdai = auth.get_company_agent_session(company_id=company_id)
    response = xdai.learn_file(
        agent_name="xdai",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    xdai = auth.get_company_agent_session(company_id=company_id)
    response = xdai.get_memories_external_sources(
        agent_name="xdai",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hxdain feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = xdai.agent_interactions.agent_memory
    else:
        memory = xdai.agent_interactions.agent_memory
    reflection = await xdai.inference(
