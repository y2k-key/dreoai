...
- xdai_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- xdai_URI: URL to the xdai server
...
def is_xdai_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("xdai_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if xdai_API_KEY:
        ...
        logging.error(
            "xdai API Key is missing. Please set the xdai_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("xdai_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                xdai = xdaiSDK(base_uri=getenv("xdai_URI"))
                xdai.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                xdai.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                xdai.add_agent(
                    agent_name="xdai",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        xdai = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> xdaiSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> xdaiSDK:
        if not company_id:
            company_id = self.company_id
        ...
        xdai.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return xdai
