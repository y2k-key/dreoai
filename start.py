...
def check_prerequisites():
    if not is_tool_installed("docker"):
        ...
        if install.lower() != "y":
            print("Docker is required to run uma. Exiting.")
            sys.exit(1)
    ...
    if not is_tool_installed("docker-compose"):
        ...
        if install.lower() != "y":
            print("Docker Compose is required to run uma. Exiting.")
            sys.exit(1)
...
def get_default_env_vars():
    ...
    return {
        ...
        "uma_PORT": "7437",
        "uma_INTERACTIVE_PORT": "3437",
        "uma_AGENT": "uma",
        "uma_BRANCH": "stable",
        "uma_FILE_UPLOAD_ENABLED": "true",
        "uma_VOICE_INPUT_ENABLED": "true",
        "uma_FOOTER_MESSAGE": "Powered by uma",
        "uma_REQUIRE_API_KEY": "false",
        "uma_RLHF": "true",
        "uma_SHOW_SELECTION": "conversation,agent",
        "uma_SHOW_AGENT_BAR": "true",
        "uma_SHOW_APP_BAR": "true",
        "uma_CONVERSATION_MODE": "select",
        "uma_SHOW_OVERRIDE_SWITCHES": "tts,websearch,analyze-user-input",
        "ALLOWED_DOMAINS": "*",
        "APP_DESCRIPTION": "A chat powered by uma.",
        "APP_NAME": "uma Chat",
        ...
        "DATABASE_NAME": "models/uma",
        "LOG_LEVEL": "INFO",
        "LOG_FORMAT": "%(asctime)s | %(levelname)s | %(message)s",
        "UVICORN_WORKERS": "10",
        "uma_AUTO_UPDATE": "true",
        ...
        "uma_ALLOW_MESSAGE_EDITING": "true",
        "uma_ALLOW_MESSAGE_DELETION": "true",
        "uma_SHOW_CHAT_THEME_TOGGLES": "",
...
def set_environment(env_updates=None):
    ...
    if env_updates:
        for key, value in env_updates.items():
            if key in env_vars:
                env_vars[key] = value
    # Ensure uma_API_KEY is set
    if env_vars["uma_API_KEY"] == "":
        env_vars["uma_API_KEY"] = "".join(
            random.choice(
    ...
    if env_vars["uma_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    if env_vars["uma_BRANCH"] != "stable":
        dockerfile = "docker-compose-dev.yml"
    else:
        dockerfile = "docker-compose.yml"
    if str(env_vars["uma_AUTO_UPDATE"]).lower() == "true":
...
if __name__ == "__main__":
    check_prerequisites()
    parser = argparse.ArgumentParser(description="uma Environment Setup")
    ...
    # Check if .env file exists and if uma_AUTO_UPDATE is not set via command line
    if not os.path.exists(".env") and "uma_AUTO_UPDATE" not in env_updates:
        auto_update = prompt_user(
            "Would you like uma to auto update when this script is run in the future? (Y for yes, N for no)",
            "y",
        )
        if auto_update.lower() == "y" or auto_update.lower() == "yes":
            ...
            auto_update = "false"
        env_updates["uma_AUTO_UPDATE"] = auto_update
