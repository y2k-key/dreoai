...
class TaskPlanInput:
    ...
    conversation_name: str = "vital Task Planning"
...
class Mutation:
    ...
    async def learn_file(self, info, agent_name: str, input: FileMemoryInput) -> bool:
        ...
        if input.company_id:
            vital = auth_manager.get_company_agent_session(company_id=input.company_id)
            response = vital.learn_file(
                agent_name="vital",
    ...
    async def submit_feedback(
        ...
        conversation = vital.conversation
        if conversation.has_received_feedback(message=input.message):
            return False

        memory = (
            vital.agent_interactions.agent_memory
            if input.positive
            else vital.agent_interactions.agent_memory
        )

        reflection = await vital.inference(
            user_input=input.user_input,
...
"""
    ...
    async def generate_dpo(self, info, agent_name: str, input: DPOInput) -> DPOResult:
        ...
        prompt, chosen, rejected = await vital.dpo(
    ...
    async def run_chain_step(
        ...
        chain_step_response = await vital(
            user=user,
            agent_name=agent_name,
            api_key=auth,
            conversation_name=input.conversation_name,
