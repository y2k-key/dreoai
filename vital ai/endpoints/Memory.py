...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        vital = auth.get_company_agent_session(company_id=external_source.company_id)
        response = vital.delete_memory_external_source(
            agent_name="vital",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    vital = auth.get_company_agent_session(company_id=company_id)
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    vital = auth.get_company_agent_session(company_id=company_id)
    response = vital.get_memories_external_sources(
        agent_name="vital",
        collection_number=collection_number,
    )
...
async def rlhf(
    ...
    if data.positive == True:
        memory = vital.agent_interactions.agent_memory
    else:
        memory = vital.agent_interactions.agent_memory
