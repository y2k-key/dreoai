...
- vital_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- vital_URI: URL to the vital server
...
def is_vital_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("vital_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    ...
    if vital_API_KEY:
        ...
        logging.error(
            "vital API Key is missing. Please set the vital_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("vital_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                vital = vitalSDK(base_uri=getenv("vital_URI"))
                vital.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                vital.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                vital.add_agent(
                    agent_name="vital",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        vital = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> vitalSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> vitalSDK:
        if not company_id:
            company_id = self.company_id
        ...
        vital.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return vital
