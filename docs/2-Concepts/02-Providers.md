...
Providers are the hosts of models or services that uma uses to interact with users.

## Logical Language Model (LLM) Providers

We're fully aware that they're called `large language models` and not `logical language models`, but our approach to prompting and chaining uses your agents primary LLM for logical reasoning in the background. This is why we refer to them as `logical language models` in the context of uma.
...
- Elevenlabs (Voice cloning TTS over API, listed under `uma` Provider)
- default: Google (gTTS)
