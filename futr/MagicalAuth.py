...
- futr_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- futr_URI: URL to the futr server
...
def is_futr_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("futr_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    futr_API_KEY = getenv("futr_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if futr_API_KEY:
        if authorization == futr_API_KEY:
        ...
        logging.error(
            "futr API Key is missing. Please set the futr_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("futr_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                futr = futrSDK(base_uri=getenv("futr_URI"))
                futr.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                futr.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                futr.add_agent(
                    agent_name="futr",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        futr = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> futrSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> futrSDK:
        if not company_id:
            company_id = self.company_id
        ...
        futr.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return futr
