...
async def get_dpo_response(
    ...
    prompt, chosen, rejected = await futr.dpo(
...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        futr = auth.get_company_agent_session(company_id=external_source.company_id)
        response = futr.delete_memory_external_source(
            agent_name="futr",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    futr = auth.get_company_agent_session(company_id=company_id)
    response = futr.learn_file(
        agent_name="futr",
        file_name=file.file_name,
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    futr = auth.get_company_agent_session(company_id=company_id)
    response = futr.get_memories_external_sources(
        agent_name="futr",
        collection_number=collection_number,
    )
...
@app.post(
    ...
    description="Submits reinforcement learning from hfutrn feedback for an interaction.",
)
async def rlhf(
    agent_name: str,
    data: FeedbackInput,
    ...
    if data.positive == True:
        memory = futr.agent_interactions.agent_memory
    else:
        memory = futr.agent_interactions.agent_memory
    reflection = await futr.inference(
