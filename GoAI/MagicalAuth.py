...
- goai_API_KEY: Encryption key to encrypt and decrypt data
- APP_URI: URL to send in the email for the user to click on
- goai_URI: URL to the goai server
...
def is_goai_admin(email: str = "", api_key: str = ""):
    if api_key == getenv("goai_API_KEY"):
        return True
...
def verify_api_key(authorization: str = Header(None)):
    goai_API_KEY = getenv("goai_API_KEY")
    authorization = str(authorization).replace("Bearer ", "").replace("bearer ", "")
    if goai_API_KEY:
        if authorization == goai_API_KEY:
        ...
        logging.error(
            "goai API Key is missing. Please set the goai_API_KEY environment variable."
        )
...
class MagicalAuth:
    def __init__(self, token: str = None):
        encryption_key = getenv("goai_API_KEY")
    ...
    def register(
        ...
        try:
            ...
            if invitation:
                ...
                session.commit()
                goai = goaiSDK(base_uri=getenv("goai_URI"))
                goai.login(email=new_user.email, otp=pyotp.TOTP(mfa_token).now())
...
<p>If you did not expect this invitation, please ignore this email.</p>""",
    ...
    def create_company(
        ...
        with get_session() as db:
            try:
                ...
                goai.login(email=company_email, otp=totp.now())
                default_agent = get_default_agent()
                goai.add_agent(
                    agent_name="goai",
                    settings=(
    ...
    def create_company_with_agent(
        ...
        goai = self.get_user_agent_session()
    ...
    def get_user_agent_session(self) -> goaiSDK:
        session = get_session()
    ...
    def get_company_agent_session(self, company_id: str = None) -> goaiSDK:
        if not company_id:
            company_id = self.company_id
        ...
        goai.login(email=f"{company_id}@{company_id}.xt", otp=totp.now())
        return goai
