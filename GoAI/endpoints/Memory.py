...
async def delete_memories_from_external_source(
    ...
    if external_source.company_id is not None:
        ...
        goai = auth.get_company_agent_session(company_id=external_source.company_id)
        response = goai.delete_memory_external_source(
            agent_name="goai",
...
async def learn_cfile(
    ...
    auth = MagicalAuth(token=authorization)
    goai = auth.get_company_agent_session(company_id=company_id)
...
async def get_cunique_external_sources(
    ...
    auth = MagicalAuth(token=authorization)
    goai = auth.get_company_agent_session(company_id=company_id)
    response = goai.get_memories_external_sources(
        agent_name="goai",
        collection_number=collection_number,
    )
...
async def rlhf(
    ...
    if data.positive == True:
        memory = goai.agent_interactions.agent_memory
    else:
        memory = goai.agent_interactions.agent_memory
